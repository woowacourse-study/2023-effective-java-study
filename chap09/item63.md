...
* coder 필드는 byte[] value를 인코딩할 때 사용되는 변수
  * LATIN1 (1바이트)
  * UTF-16 (기본값2바이트)
...
### intern()
* String을 리터럴로 선언 시 intern() 메서드를 호출, 
...
* 이때 새로운 byte[]를 생성할 때 기존 용량에 2배를 지정하여 문자열 추가를 효율적으로 문자열 추가를 한다.
...
```
* 기본전략은 `MH_INLINE_SIZE_EXACT` 이 사용하게 되는데, 
